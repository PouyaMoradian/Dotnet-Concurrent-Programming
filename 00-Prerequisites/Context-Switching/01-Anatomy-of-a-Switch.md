...
3. **Save floating-point / SIMD state.** XMM/YMM (and ZMM on AVX-512) registers are ~512 B to 2 KB depending on extension. Both Linux and Windows use **lazy FPU save** (FPU = Floating-Point Unit, which on modern x86 also covers the SIMD vector registers): the FPU state is only saved if the new thread actually touches it. On hot SIMD workloads this isn't lazy any more — the kernel sees every thread touched SIMD and pays the cost.
